...
class PositionDMP():
    ...
    def __init__(self, n_bfs=10, alpha=480.0, beta=None, cs_alpha=None, cs=None):
        self.n_bfs = n_bfs
        self.alpha = alpha
        self.beta = beta if beta is not None else self.alpha / 4
        self.cs = cs if cs is not None else CanonicalSystem(
        ...
        self.p0 = np.zeros(3)
        # self.gp = np.zeros(3)
        self.gp = np.array([-0.077089, -0.295877, 0.433379])
        # self.p0 = np.array([-0.0771, -0.30312, 0.45447])
        # self.gp = np.array([-0.077093, -0.285867, 0.433387])
        # self.p0 = np.array([-0.077093, -0.285867, 0.433387])
        # self.gp = np.array([-0.077093, -0.285867, 0.433387])
    ...
    def step(self, x, dt, tau):
        ...
        self.ddp = self.alpha / tau**2 * (self.beta *
                                          (self.gp - self.p) - tau * self.dp) + fp(x) / tau**2
        # self.ddp = -self.alpha * (self.beta * (self.gp - self.p) - tau * self.dp) + self.Dp.dot(
        #     self.w.dot(np.exp(-self.h * (x - self.c)**2)) / np.exp(-self.h *
        #                                                            (x - self.c)**2).sum() * x)
    ...
    def train(self, positions, ts, tau):
        ...
        def forcing(j):
            return Dp_inv.dot(tau**2 * dd_p[j] - self.alpha * (self.beta *
                                                               (self.gp - p[j]) - tau * d_p[j]))
